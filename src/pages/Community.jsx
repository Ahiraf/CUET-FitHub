...
import { getAnnouncements, getBuddyConnections, setBuddyConnections } from '../api';
...
export default function Community() {
  ...
  const [connected, setConnected] = useState(() => getBuddyConnections(user));
  const [lang, setLang] = useState('en');
  const announcements = getAnnouncements();

  const toggleBuddy = (b) => {
    const wasIn = connected.includes(b.id);
    const next = wasIn ? connected.filter((id) => id !== b.id) : [...connected, b.id];
    setConnected(next);
    setBuddyConnections(user, next);
    showToast(wasIn ? `Removed ${b.name} as a buddy.` : `Buddy request sent to ${b.name}.`);
  };
          ...
          {buddies.map((b) => (
            <article className="info-card" key={b.id}>
              ...
              <div className="ic-foot">
                <span style={{ color: '#9aa4b4', fontSize: 10 }}>{connected.includes(b.id) ? 'Request sent' : 'Similar goals'}</span>
                <button className={`btn sm ${connected.includes(b.id) ? 'ghost' : ''}`} onClick={() => toggleBuddy(b)} type="button">
                  {connected.includes(b.id) ? 'Connected' : 'Connect'}
                </button>
              </div>
            </article>
          ))}
